...
global cap1
global cap2
cap1 = cv2.VideoCapture(os.path.join(os.path.dirname(__file__), "test2.mp4"))
cap2 = cv2.VideoCapture(os.path.join(os.path.dirname(__file__), "test2.mp4"))
...
def show_vid():                                       
    ...
    if not flag1 or frame1 is None:
        print("Major error! Cannot read frame from video")
        return
...
def show_vid2():
    ...
    if not flag2 or frame2 is None:
        print("Major error2! Cannot read frame from video")
        return
